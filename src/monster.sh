...
case $stage in
  ...
  1) # ═══ たまご ═══ 手作業デザイン・コロコロ転がる
    # シルエットで卵型を表現（上狭・中広・下やや広 = 卵形）
    # ▄▀ 半ブロックで丸みを出す
    e=$(fg 225 200 170)   # 卵メイン（温かみのあるベージュ）
    h=$(fg 245 235 215)   # ハイライト（明るいクリーム）
    s=$(fg 190 165 135)   # シャドウ（暗めベージュ）
    sp=$(fg 175 105 70)   # 模様の点（赤茶）
    case $walk_frame in
      0) # 正面
        art[0]="    ${e}▄██▄${R}"
        art[1]="   ${h}█${e}█${sp}◦${e}█${s}██${R}"
